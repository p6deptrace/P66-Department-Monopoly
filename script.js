...
function tileHtml(space, teams) {
  ...
  const image = !isStart && space.imageUrl ? `<img class="tile-img" src="${space.imageUrl}" alt="${escapeHtml(space.name)}" loading="eager" onerror="this.style.display='none'" />` : "";
  const startVisual = isStart ? `<div class="start-visual"><div class="start-badge"><div class="start-arrow">▶</div><div class="start-word">Begin</div></div></div>` : "";
  return `
